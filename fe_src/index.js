...
const addTempFeatures = (data, target) => {
  JSON.parse(data).forEach((item, i) => {
    const markerEl = getMarker(item.IconUrl);
    const marker = new mapboxgl.Marker(markerEl)
      .setLngLat([item.Lat, item.Lon])
      .addTo(map);
    const delay = Math.random()*2;
    TweenMax.from(markerEl, .35, {ease: Back.easeOut, alpha:0, scale:0, delay: delay});
    TweenMax.to(markerEl, .35, {ease: Back.easeIn, scale:0, delay: delay+10, onComplete:removeMarker, onCompleteParams:[marker, target]});
  })
}

const removeMarker = (marker, target) => {
  marker.remove();
}
